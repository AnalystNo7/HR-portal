...
**LLM:** любой OpenAI-совместимый API (`/chat/completions`), клиент на нативном fetch; конфиг в БД (`/admin/llm`, пресеты) с fallback на env `LLM_*`. Провайдер — Gonka/OpenGNK (`api.proxy.gonka.gg`, потолок вывода 16384; доступны MiniMax-M2.7 и Kimi-K2.6). Генерация может идти **по частям — от 1 до 8 запросов, последовательно**: при 6–7 по одной паре на запрос дробится `groupComparison`, при 8 — ещё и `externalComparison`. Лимит токенов и таймаут каждой части — в пресете; там же подсказка, какой потолок вывода нужен выбранному числу частей.
**Инфраструктура:** PostgreSQL 16-alpine, Keycloak 24.0 (realm `hr-portal`, клиент `hr-portal-app`, **боевой режим `start`**), Docker Compose. Прод — Dokploy + **Traefik** (наружу только он, по HTTPS; порты сервисов на хост **не публикуются**). **TLS — на Traefik сервера (Let's Encrypt, автопродление); домен ведёт на сервер напрямую, без балансировщика** (см. Known issues). Роутинг по путям `/` → frontend :3001, `/api` → backend :4000, `/auth` → keycloak :8080. Таймаут долгих запросов (LLM-генерация) — **≥1300с** (nginx-локалка задан; Traefik на проде — вручную, см. Known issues).
...
| oc360/report | `backend/src/oc360/report/` | AI-отчёт: `llm.client.ts` (OpenAI-совм. клиент, срез `<think>`, авто-повтор при таймауте, повтор при 429 и при **5xx/524 шлюза**, код сетевой ошибки из `e.cause` в логе, max_tokens по галочке, таймаут по частям); `report.prompt.ts` (методика v1.2 + документы + защищённая JSON-схема; `partsForCount` → **`ReportPart {keys, pairs?, externalPairs?}`**, схема разделов-пар собирается по списку пар); `report.types.ts` (`normalizeSections`, `hiddenBlocks`, externalComparison); `report.service.ts` (generate/update/reset; **замок от двойной генерации** — `Set` по subjectId → 409; склейка частей: разделы-пары **дописываются**, остальные ключи присваиваются; снимки `preGenSnapshot`/`initialSnapshot`) |
...
| /admin/llm | `frontend/app/admin/llm/page.tsx` | Список пресетов и форма: адрес, ключ, модель, температура, лимит ответа, **число запросов 1–8 с подсказкой требуемого потолка вывода** (`CEILING_BY_PARTS`), таймаут на каждую часть; «Проверить подключение» |
...
- **Домен ведёт на сервер напрямую, TLS выпускает Traefik** (Let's Encrypt, автопродление; хранилище `/etc/dokploy/traefik/dynamic/acme.json`). Условия автопродления — A-запись на сервер и открытый порт 80. Балансировщик Timeweb выведен из цепочки: услуга «SSL из панели» у этого хостера создаёт балансировщик и вешает сертификат на него, а для одного сервера это лишняя точка отказа (инцидент 2026-07-28).
- **IP сервера в репозитории не хранить** — при пересоздании/восстановлении машины он меняется. Актуальные адреса, аварийная диагностика и порядок восстановления — в `VPS_INSTRUCTION.md` (вне репозитория).
...
- Список моделей провайдера проверять через `GET {base}/models`, не по документации. Там же смотреть `max_tokens` — потолок вывода задаёт **шлюз**, а не модель, и у разных брокеров одной сети он разный (Gonka 16384, отклонённый GonkaRouter — 4096). Без явного `max_tokens` шлюз режет по своему дефолту (у Gonka 3072), который ниже его же потолка.
- Число частей подбирать под потолок провайдера: подсказка в форме пресета даёт ориентир, факт проверяется по `completion` при `finish_reason=length` в логе.
...
- «LLM недоступен: fetch failed (КОД)» — сеть **до** провайдера, а не сам провайдер: `ENOTFOUND`/`ETIMEDOUT` = блокировка (для локальной разработки нужен VPN), `ECONNREFUSED` = сервис лежит. Сверять доступность с двух точек — локально и с прод-сервера.
...
---
_Обновлено 2026-07-28 по состоянию ветки `mvp2`: генерация до 8 частей с дроблением разделов-пар, повтор при 5xx и код сетевой ошибки в логе, подсказки потолка в `/admin/llm`; GonkaRouter проверен и отклонён (DECISIONS 2026-07-27); домен переведён напрямую на сервер с TLS на Traefik после инцидента с удалением сервера (LESSONS 2026-07-28)._
