...
**В панели Dokploy:** проект `hr-portal` → сервис `hr-portal-stack` → вкладка **Logs**.
Стек из четырёх контейнеров, поэтому нужный выбирается из списка: `postgres`,
`keycloak`, `backend`, `frontend`. Поток идёт вживую, есть поиск и выбор числа строк.

Вкладка **Deployments** — это другое: там логи *сборки* (docker build, `npm ci`,
`next build`). Ошибки компиляции ищутся там, ошибки работающего приложения — в **Logs**.

**Это те же логи, что в cmd локально.** Приложение пишет в stdout/stderr, в контейнере
тот же вывод перехватывает Docker — те же строки Nest, те же `logger.error` из
`llm.client.ts`. Отличия от локального запуска:
- нет цветов и живой перерисовки (не TTY);
- история обнуляется при Redeploy — контейнер пересоздаётся;
- в проде `start:prod`, поэтому нет подробностей watch-режима.

**По SSH на сервере** — когда нужно грепать и смотреть историю:
```bash
docker ps                                  # найти имя контейнера
docker logs -f --tail 200 <container>      # вживую, последние 200 строк
docker logs --since 30m <container> | grep -i "llm\|error"   # разбор инцидента
```

**Разбор неудачной AI-генерации отчёта** — сервис `backend`: ушёл ли запрос к провайдеру,
какой HTTP-код вернулся, был ли `finish_reason=length` (не хватило `max_tokens` — поднять
лимит или число частей в `/admin/llm`), был ли обрыв по таймауту Traefik (см. Known issues
в `harness/PROJECT.md`). Сообщение `LLM недоступен: fetch failed (КОД)` означает проблему
сети *до* провайдера, а не самого провайдера.

> Размер журналов не ограничен: драйвер `json-file` пишет без ротации, на долгоживущем
> сервере это ест диск. Лечится блоком `logging` (`max-size`, `max-file`) для сервисов
> в `docker-compose.yml` — сейчас не настроено.
