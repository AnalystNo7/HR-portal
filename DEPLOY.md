...
(`LlmService.TIMEOUT_MS = 300s`), при таймауте backend делает 1 автоповтор
(ноды Gonka нестабильны) — суммарно генерация может держать соединение до ~11 минут.
Таймаут чтения reverse-proxy для пути `/api` должен быть больше суммарного времени:
в `nginx/nginx.conf` задан `proxy_read_timeout 700s`; при использовании Traefik в Dokploy
задайте таймаут ответа ≥ 700s (иначе прокси оборвёт запрос раньше, чем модель ответит).
