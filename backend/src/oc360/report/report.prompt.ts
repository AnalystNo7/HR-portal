...
- В blindSpots и hiddenPotential включай только компетенции с |Δ| ≥ 0.6; если таких нет —
  верни пустой массив. Поле "text" — абзац «Подтверждение из комментариев»: опора на
  открытые ответы (парафраз, без имён); поле "conclusion" — абзац «Вывод» с трактовкой
  разрыва и рекомендацией. Цифры оценок не повторяй в text — они выводятся отдельно.
 ...
 */
export const TECHNICAL_PROMPT = `Язык текста:
...
- Ключи JSON и значения-перечисления схемы (pair: SELF_MANAGER…, kind: CONSENSUS|BLIND_SPOT|HIDDEN_POTENTIAL) оставляй ровно как в схеме — на них запрет латиницы НЕ распространяется.
