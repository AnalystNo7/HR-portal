...
export interface IndicatorTpl { id: string; competencyId: string; text: string; description?: string | null; isReverse?: boolean; order: number; }
...
export interface Report360 {
  ...
  /** Доступен ли откат к предыдущей версии (перед последней генерацией). */
...
export const update360Indicator = (id: string, dto: { text?: string; description?: string | null; isReverse?: boolean; order?: number }) => fetchApi<IndicatorTpl>(`/oc360/template/indicators/${id}`, { method: 'PUT', body: JSON.stringify(dto) });
...
export const reset360Report = (id: string, sid: string, mode: ReportResetMode) => fetchApi<Report360 | null>(`/oc360/cycles/${id}/subjects/${sid}/report/reset`, { method: 'POST', body: JSON.stringify({ mode }) });
