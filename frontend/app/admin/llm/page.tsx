...
/**
 * Ориентир «число частей → потолок вывода провайдера»: по самой объёмной части каждой
 * раскладки (замеры на боевых прогонах) плюс запас на многословные модели. Не гарантия —
 * модель, пишущая длиннее замеренной, может не влезть и в эти цифры.
 */
const CEILING_BY_PARTS: Record<number, number> = {
  1: 16000, // весь отчёт одним ответом
  2: 9000,
  3: 6000,
  // до восьми частей ограничивает сравнение с внешними группами: цельным оно
  // не уложилось в 4096 (замер: обрыв по лимиту)
  4: 5000,
  5: 5000,
  6: 5000,
  7: 5000,
  // оба раздела-пары разрезаны; ограничивает блок «слепые + скрытые» (замер 3264)
  8: 4000,
};
