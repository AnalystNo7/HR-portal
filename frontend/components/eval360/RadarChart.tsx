...
export function RadarChart({ axes, series, min, max, size = 680, showValues = false }: {
    ...
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ maxWidth: '100%', height: 'auto', overflow: 'visible' }}>
      ...
      {axes.map((a, i) => {
        ...
        const y0 = p.y < cy - 8 ? p.y - (n - 1) * LINE_H
          ...
          : p.y - ((n - 1) * LINE_H) / 2;
        return (
          <text key={`a${i}`} textAnchor={anchor} fontSize={15} fill="var(--gpc-gray-700)">
            {nameLines.map((ln, j) => (
              <tspan key={j} x={p.x} y={y0 + j * LINE_H} dominantBaseline="middle" fontWeight={600}>{ln}</tspan>
            ))}
            <tspan x={p.x} y={y0 + nameLines.length * LINE_H} dominantBaseline="middle" fontSize={13} fill="var(--gpc-gray-500)">средний балл {a.value == null ? '—' : a.value.toFixed(1)}</tspan>
          </text>
        );
      })}
      ...
      {hover && (
        <g pointerEvents="none">
          <rect x={hover.x - 23} y={hover.y - 36} width={46} height={24} rx={4} fill="var(--gpc-blue-800)" />
          <text x={hover.x} y={hover.y - 24} textAnchor="middle" dominantBaseline="middle" fontSize={15} fontWeight={600} fill="#fff">{hover.text}</text>
        </g>
      )}
    </svg>
  );
